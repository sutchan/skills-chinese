...
本文档提供了为 MCP 服务器创建全面评估的指导。评估测试 LLM 是否能仅使用提供的工具，有效利用您的 MCP 服务器回答真实、复杂的问题。
...
MCP 服务器质量的衡量标准不是服务器实现工具的好坏或全面程度，而是这些实现（输入/输出模式、文档字符串/描述、功能）如何使没有其他上下文且仅访问 MCP 服务器的 LLM 能够回答真实且困难的问题。

## 评估概述

创建 10 个人类可读的问题，这些问题仅需要只读、独立、非破坏性和幂等操作来回答。每个问题应：
- 真实
- 清晰简洁
- 无歧义
- 复杂，可能需要数十个工具调用或步骤
...
5. **问题可能需要大量分页**
   - 可能需要分页浏览多页结果
   - 可能需要查询旧数据（1-2 年前的过时数据）以找到小众信息
   - 问题必须困难

6. **问题必须要求深入理解**
   - 而非表面知识
   - 可能将复杂想法作为需要证据的真/假问题提出
   - 可能使用选择题格式，其中 LLM 必须搜索不同的假设
...
8. **问题应压力测试工具返回值**
   - 可能引发工具返回大型 JSON 对象或列表，使 LLM 不堪重负
   - 应要求理解多种数据模态：
     - ID 和名称
     - 时间戳和日期时间（月、日、年、秒）
...
1. **答案必须可通过直接字符串比较验证**
   ...
   - 示例："使用 YYYY/MM/DD 格式。"、"回答 True 或 False。"、"回答 A、B、C 或 D，仅此而已。"
   - 答案应该是单一可验证的值，例如：
...
1. **检查 XML 文件**以了解模式
2. **加载每个任务指令**并使用 MCP 服务器和工具并行尝试自己解决任务，从而确定正确答案
3. **标记**任何需要写入或破坏性操作的操作
4. **积累所有正确答案**并替换文档中的任何错误答案
5. **删除**任何需要写入或破坏性操作的 `<qa_pair>`

记住要并行解决任务以避免耗尽上下文，然后积累所有答案并在最后对文件进行更改。

## 创建高质量评估的提示

1. **在生成任务前深思熟虑并提前规划**
2. **在机会出现时并行处理**以加快流程并管理上下文
3. **关注人类实际想要完成的真实用例**
4. **创建挑战性问题**以测试 MCP 服务器能力的极限
5. **通过使用历史数据和已关闭概念确保稳定性**
6. **通过使用 MCP 服务器工具自己解决问题来验证答案**
7. **基于过程中学习到的内容进行迭代和完善**
...
- **sse/http 传输**：在运行评估之前，您必须单独启动 MCP 服务器。脚本连接到已在指定 URL 上运行的服务器。
...
sse/http 选项：
  -u, --url             MCP 服务器 URL
  -H, --header          'Key: Value' 格式的 HTTP 头部
```
