...
使用 `@mcp.tool` 装饰器和 Pydantic 模型定义工具用于输入验证：
...
async def service_tool_name(params: ServiceToolInput) -> str:
    ...
    此工具在服务上执行特定操作。它在处理之前使用 ServiceToolInput Pydantic 模型验证所有输入。
...
async def search_users(params: UserSearchInput) -> str:
    ...
    示例：
        ...
        - 不要使用场景：当您需要创建用户时（请改用 example_create_user）
        - 不要使用场景：当您有用户 ID 并需要完整详情时（请改用 example_get_user）
...
FastMCP 可以自动向工具注入 `Context` 参数，以实现日志记录、进度报告、资源读取和用户交互等高级功能：
...
- [ ] 所有 Pydantic Field 都有明确的类型和带有约束的描述
- [ ] 所有工具都有包含明确输入/输出类型的综合文档字符串
