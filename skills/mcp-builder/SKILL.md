---
name: mcp-builder
description: 用于创建高质量 MCP（模型上下文协议）服务器的指南，使 LLM 能够通过设计良好的工具与外部服务交互。当构建 MCP 服务器以集成外部 API 或服务时使用，无论是在 Python（FastMCP）还是 Node/TypeScript（MCP SDK）中。
...
创建 MCP（模型上下文协议）服务器，使 LLM 能够通过设计良好的工具与外部服务交互。MCP 服务器的质量通过其使 LLM 完成实际任务的能力来衡量。
...
**API 覆盖 vs. 工作流工具：**
平衡全面的 API 端点覆盖与专门的工作流工具。工作流工具对于特定任务可能更方便，而全面的覆盖给代理提供了组合操作的灵活性。性能因客户端而异——一些客户端受益于结合基本工具的代码执行，而其他客户端则更适合高级工作流。不确定时，优先考虑全面的 API 覆盖。

**工具命名和可发现性：**
清晰、描述性的工具名称帮助代理快速找到正确的工具。使用一致的前缀（例如，`github_create_issue`、`github_list_repos`）和面向动作的命名。

**上下文管理：**
代理受益于简洁的工具描述和过滤/分页结果的能力。设计返回重点突出、相关数据的工具。一些客户端支持代码执行，可以帮助代理高效过滤和处理数据。
...
- **传输**：远程服务器使用可流式 HTTP，使用无状态 JSON（比有状态会话和流式响应更简单，易于扩展和维护）。本地服务器使用 stdio。
...
使用评估来测试 LLM 是否能有效使用您的 MCP 服务器回答现实、复杂的问题。
